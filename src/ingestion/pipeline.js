...
export async function importYouTubeVideo(url, courseMeta) {
  try {
    ...
    return { ok: false, error: `Video import failed: ${err.message}` };
  }
}
...
export async function importYouTubeURLList(rawText, courseMeta, onProgress) {
  try {
    if (!rawText?.trim()) return { ok: false, error: 'Paste at least one YouTube URL.' };
    ...
    return { ok: false, error: `URL list import failed: ${err.message}` };
  }
}
...
export async function importYouTubePlaylist(url, courseMeta, onProgress) {
  try {
    if (!url?.trim()) return { ok: false, error: 'Paste a YouTube playlist URL.' };
    ...
    if (doc.documentElement.nodeName === 'parsererror') {
      throw new Error('Invalid RSS feed returned');
    }
